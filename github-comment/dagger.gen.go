...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "GithubComment":
		...
		case "":
			...
			return New(ctx, githubToken, messageId, owner, repo, issue), nil
		default:
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("GithubComment").
					WithFunction(
						dag.Function("Create",
							dag.TypeDef().WithKind(StringKind).WithOptional(true)).
							...
							WithArg("body", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("Delete",
					...
					WithFunction(
						dag.Function("React",
							...
							WithArg("kind", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "\"+1\", \"-1\", \"laugh\", \"confused\", \"heart\", \"hooray\", \"rocket\", or \"eyes\"."})).
					WithField("GithubToken", dag.TypeDef().WithObject("Secret")).
					WithField("MessageID", dag.TypeDef().WithKind(StringKind)).
					WithField("Owner", dag.TypeDef().WithKind(StringKind)).
					WithField("Repo", dag.TypeDef().WithKind(StringKind)).
					WithField("Issue", dag.TypeDef().WithKind(IntegerKind)).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("GithubComment")).
							WithArg("githubToken", dag.TypeDef().WithObject("Secret")).
							WithArg("messageID", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"github.com/aluzzardi/daggerverse/github-comment\"")}).
							WithArg("owner", dag.TypeDef().WithKind(StringKind)).
							WithArg("repo", dag.TypeDef().WithKind(StringKind)).
							WithArg("issue", dag.TypeDef().WithKind(IntegerKind)))), nil
	default:
