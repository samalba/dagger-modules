...
// The `InlinePythonID` scalar type represents an identifier for an object of type InlinePython.
type InlinePythonID string
...
type WithInlinePythonFunc func(r *InlinePython) *InlinePython

// With calls the provided function with current InlinePython.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *InlinePython) With(f WithInlinePythonFunc) *InlinePython {
	return f(r)
}

func (r *InlinePython) Code(code string) *Container {
	q := r.q.Select("code")
	q = q.Arg("code", code)
...
func (r *InlinePython) Ctr() *Container {
	q := r.q.Select("ctr")
...
// A unique identifier for this InlinePython.
func (r *InlinePython) ID(ctx context.Context) (InlinePythonID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.q.Select("id")

	var response InlinePythonID
...
// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *InlinePython) XXX_GraphQLType() string {
	return "InlinePython"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *InlinePython) XXX_GraphQLIDType() string {
	return "InlinePythonID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *InlinePython) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *InlinePython) MarshalJSON() ([]byte, error) {
	id, err := r.ID(context.Background())
	if err != nil {
		return nil, err
	}
...
func (r *InlinePython) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *dag.LoadInlinePythonFromID(InlinePythonID(id))
	return nil
}

// fixme: add usage
func (r *InlinePython) WithPackage(name string) *InlinePython {
	q := r.q.Select("withPackage")
	q = q.Arg("name", name)
...
// fixme: add usage
func (r *InlinePython) WithPackages(packages []string) *InlinePython {
	q := r.q.Select("withPackages")
...
// InlinePythonOpts contains options for Client.InlinePython
type InlinePythonOpts struct {
	BaseImage string
}

func (r *Client) InlinePython(opts ...InlinePythonOpts) *InlinePython {
	q := r.q.Select("inlinePython")
	for i := len(opts) - 1; i >= 0; i-- {
...
// Load a InlinePython from its ID.
func (r *Client) LoadInlinePythonFromID(id InlinePythonID) *InlinePython {
	q := r.q.Select("loadInlinePythonFromID")
	q = q.Arg("id", id)
